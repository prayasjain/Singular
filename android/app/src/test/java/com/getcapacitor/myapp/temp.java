...
public class temp {
    ...
    @Test
    public void temp() {
        Pattern amountPattern = Pattern.compile("(?:balance\\sis\\snow\\s|(?:avb?l\\s+)?bal\\s*:?-?)\\s*(?:inr|rs\\.)\\s*([\\d,]+\\.\\d{0,2})");
...
//        }
        for (String text: Arrays.asList(text1,text2,text3,text4,text5,text6)) {
